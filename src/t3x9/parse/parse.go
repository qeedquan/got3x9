...
func (p *Parser) program() *ast.Prog {
	...
	p.add(&ast.Symbol{Ident: "t.memwrite", Len: 3, Value: &ast.ConstExpr{Value: 3}}, ast.FUNC)
...
func (p *Parser) evalconst(e ast.Expr) int {
	...
	case *ast.BasicLit:
		...
		case lex.INTEGER:
			...
			ri, _ := strconv.ParseInt(v, 0, 64)
			r = int(ri)
		case lex.SYMBOL:
			ri, _ := strconv.ParseInt(e.Value, 0, 64)
			r = int(ri)
		default:
...
func (p *Parser) fncall(name *ast.Symbol, fun ast.Expr) *ast.CallExpr {
	...
	if y != nil && y.Type != ast.FUNC && y.Type != ast.DECL {
